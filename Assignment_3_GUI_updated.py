...
class AIApp(tk.Tk):
    def __init__(self):
        ...
        # ---------------- Input Section ----------------
        input_frame = ttk.LabelFrame(self, text="User Input Section: Enter a brief description of the image you want to create if you've chosen the Text to Image model. If you want to summarize text, you can type it in or upload a text file.")
    ...
    def inference_runner(self):
        ...
        if self.model_name=="Text-to-Image":
            ...
            self.explain_box.insert(tk.END,"Inheritance: Used in Text model and Image model Handlers, inheriting from Model Handler class\nEncapsulation: Implemented on the inference api key with PROTECTED status\nPolymorphism: Utilized on the run inference function\nDecorators: Utilized on the log action and ensure input functions\Method overriding:??")
